...
function generarEncabezado(datos) {
  ...
  return [
    new Paragraph({
      children: [new TextRun({ text: "--- INSTRUMENTO: _______________", bold: true })],
      spacing: { after: 100 }
    }),
    new Paragraph({
      children: [new TextRun("--- TOMO: _____ LIBRO: _____")],
      spacing: { after: 100 }
    }),
    new Paragraph({
      children: [
        ...
        new TextRun(`, publicado el ${cfg.suplente.convenio.fecha}, en el ${cfg.suplente.convenio.publicacion}, hago constar: `)
      ],
      spacing: { after: 300 },
...
function generarActosJuridicos(datos) {
  ...
  paragraphs.push(new Paragraph({
    children: [
      new TextRun({ text: `--- I.- EL CONTRATO DE CESIÓN DEL 100% (CIEN POR CIENTO) DE DERECHOS FIDEICOMISARIOS DEL FIDEICOMISO IDENTIFICADO ADMINISTRATIVAMENTE BAJO EL NÚMERO ${numFideicomiso} (${numFideicomisoLetra}), que ante mí otorgan y formalizan:`, bold: true })
    ],
  ...
  paragraphs.push(new Paragraph({
    children: [
      ...
      new TextRun(`, quienes comparecen por instrucciones de ${conjugarCedente(cedentes)}. `)
    ],
    spacing: { after: 300 },
    alignment: AlignmentType.JUSTIFIED
  }));
  ...
  return [
    new Paragraph({
      children: [
        ...
        new TextRun(`, en su calidad de intérprete del idioma español al inglés para los efectos del presente otorgamiento, quien acepta y protesta el cargo conferido, comprometiéndose a desempeñarlo legal y fielmente. `)
      ],
      spacing: { after: 300 },
      alignment: AlignmentType.JUSTIFIED
    })
...
function generarAntecedentes(datos) {
  ...
  paragraphs.push(new Paragraph({
    children: [new TextRun({ text: "--- A N T E C E D E N T E S ---", bold: true })],
    ...
    fideicomiso.cesionesPrevias.forEach((cesion, idx) => {
      paragraphs.push(new Paragraph({
        children: [
          ...
          new TextRun(`Por escritura pública número ${cesion.escritura || '[NÚMERO]'}, de fecha ${formatearFecha(cesion.fecha || new Date().toISOString().split('T')[0])}, ${cesion.cedente || '[CEDENTE]'} cedió el 100% de los derechos fideicomisarios a favor de ${cesion.cesionario || '[CESIONARIO]'}. `)
        ],
        spacing: { after: 200 },
        alignment: AlignmentType.JUSTIFIED
      }));
  ...
  paragraphs.push(new Paragraph({
    children: [
      ...
      new TextRun(`, inscrito en el Registro Público de la Propiedad bajo el folio real electrónico ${inmueble.folioReal || '[FOLIO]'} (${convertirALetra(inmueble.folioReal || '')}). `)
    ],
    spacing: { after: 200 },
    alignment: AlignmentType.JUSTIFIED
  }));
  ...
  if (inmueble.descripcionLegal) {
    paragraphs.push(new Paragraph({
      children: [new TextRun(inmueble.descripcionLegal)],
      spacing: { after: 200 },
      alignment: AlignmentType.JUSTIFIED
    }));
  }
...
function generarGenerales(datos) {
  ...
  paragraphs.push(new Paragraph({
    children: [new TextRun({ text: "--- G E N E R A L E S ---", bold: true })],
    spacing: { before: 300, after: 200 },
    alignment: AlignmentType.CENTER
  }));
...
function generarCertificaciones(datos) {
  ...
  return [
    new Paragraph({
      children: [new TextRun({ text: "--- C E R T I F I C A C I O N E S ---", bold: true })],
      spacing: { before: 300, after: 200 },
      alignment: AlignmentType.CENTER
    }),
    new Paragraph({
      children: [
        ...
        new TextRun(`Que me fueron exhibidos los documentos que acreditan la legal existencia, capacidad y personalidad de los comparecientes, mismos que obran agregados al apéndice del presente instrumento. `)
      ],
      spacing: { after: 200 },
      alignment: AlignmentType.JUSTIFIED
    }),
    new Paragraph({
      children: [
        ...
        new TextRun(`Que los comparecientes tienen a mi juicio capacidad legal para contratar y obligarse, y que sus voluntades se encuentran exentas de todo vicio. `)
      ],
      spacing: { after: 200 },
      alignment: AlignmentType.JUSTIFIED
    }),
    new Paragraph({
      children: [
        ...
        new TextRun(`Que leí en voz alta el presente instrumento a los comparecientes, les expliqué su valor y consecuencias legales, y habiéndolo entendido en su contenido y alcances, lo ratifican y firman, firmando también el intérprete en su caso. `)
      ],
      spacing: { after: 200 },
      alignment: AlignmentType.JUSTIFIED
    }),
    new Paragraph({
      children: [new TextRun(`DOY FE. `)],
      spacing: { after: 400 },
      alignment: AlignmentType.CENTER
    })
  ];
...
export async function generarCesionFideicomiso(datos) {
  const children = [];
  
  // Encabezado
  children.push(...generarEncabezado(datos));
  
  // Actos jurídicos (comparecientes)
  children.push(...generarActosJuridicos(datos));
  
  // Intérprete (si aplica)
  children.push(...generarInterprete(datos));
  
  // Antecedentes
  children.push(...generarAntecedentes(datos));
  
  // Contraprestación
  children.push(...generarContraprestacion(datos));
  
  // Fideicomisarios sustitutos
  children.push(...generarFideicomisariosSustitutos(datos));
  
  // Generales
  children.push(...generarGenerales(datos));
  
  // Certificaciones
  children.push(...generarCertificaciones(datos));
  ...
  const doc = new Document({
    ...
    sections: [{
      properties: {
        page: {
          size: { width: 12240, height: 20160 }, // Oficio
...
export { CONFIG_NOTARIO, numeroALetras, formatearFecha, convertirALetra };
